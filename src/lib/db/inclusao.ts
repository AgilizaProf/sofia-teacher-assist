...
export async function bulkDeleteInclusaoStudents(ids: string[]): Promise<void> {
...
export async function bulkAssignTurmaInclusao(
  ids: string[],
  turma: string,
): Promise<void> {
  if (ids.length === 0) return;
  const { error } = await supabase
    .from("alunos_inclusao")
    .update({ turma, updated_at: new Date().toISOString() })
    .in("id", ids);
  if (error) throw error;
}