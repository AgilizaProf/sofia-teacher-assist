    ...
    <div className="ctx-chips">
      ...
      {tutorialOpen && (() => {
        const steps = [
          ...
          { title: "Sugestões e tarefas rápidas", body: "Logo abaixo da saudação, você encontra cards de tarefas (pareceres, planos, inclusão, avaliação). Clique em uma das tarefas para iniciar uma conversa já direcionada." },
          { title: "Contexto ativo", body: "Use o painel de contexto (acima do chat) para informar turma, bimestre, foco, estilo, recursos e o que evitar. Quanto mais contexto, mais personalizadas ficam as respostas da Sofia." },
          { title: "Histórico de conversas", body: "Na lateral (ou no botão 'Histórico' no mobile) ficam todas as suas conversas anteriores, agrupadas por hoje, semana e mais antigas. Clique em qualquer uma para retomar." },
          ...
          { title: "Como pedir bem", body: "Seja específico(a): cite a turma, o aluno (quando aplicável), o objetivo e o formato desejado (ex.: 'parecer bimestral em tópicos', 'plano de aula 50 minutos'). A Sofia já sabe seu contexto." },
        ];
        const total = steps.length;
        const s = steps[Math.min(tutorialStep, total - 1)];
        ...
        return (
          ...
          >
            <div
              ...
              style={{ background: "var(--card, #fff)", color: "var(--fg, #0f172a)", borderRadius: 16, maxWidth: 520, width: "100%", padding: 24, boxShadow: "0 20px 60px rgba(0,0,0,.25)" }}
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 0.6, color: "var(--muted, #64748b)" }}>
                  TUTORIAL · {tutorialStep + 1}/{total}
                </span>
              ...
              <p style={{ margin: 0, fontSize: 14, lineHeight: 1.55, color: "var(--muted, #475569)" }}>{s.body}</p>
              <div style={{ display: "flex", gap: 6, marginTop: 18, marginBottom: 18 }}>
                {steps.map((_, i) => (
                  <span key={i} style={{ height: 4, flex: 1, borderRadius: 2, background: i <= tutorialStep ? "var(--primary, #4f46e5)" : "rgba(148,163,184,.3)" }} />
                ))}
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                <button
                  ...
                  style={{ padding: "8px 14px", borderRadius: 10, border: "1px solid rgba(148,163,184,.4)", background: "transparent", cursor: tutorialStep === 0 ? "not-allowed" : "pointer", opacity: tutorialStep === 0 ? 0.5 : 1, color: "inherit" }}
                >
                  Voltar
                </button>
                {tutorialStep < total - 1 ? (
                  <button
                    onClick={() => setTutorialStep((n) => Math.min(total - 1, n + 1))}
                    style={{ padding: "8px 14px", borderRadius: 10, border: 0, background: "var(--primary, #4f46e5)", color: "#fff", cursor: "pointer", fontWeight: 600 }}
                  >
                    Próximo
                  </button>
                ) : (
...
@media(min-width:1101px){
  .ai-app{grid-template-columns:240px 1fr 320px;}
  .ai-app.collapsed{grid-template-columns:240px 1fr 56px;}
}
